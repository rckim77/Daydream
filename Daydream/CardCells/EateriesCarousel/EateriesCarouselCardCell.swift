//
//  EateriesCarouselCardCell.swift
//  Daydream
//
...
final class EateriesCarouselCardCell: UICollectionViewCell {
    ...
    static let reuseIdentifier = "eateriesCarouselCardCell"
    ...
    func configure(eatery: Eatable) {
        ...
        if let cachedImage = ImageCache.shared.get(forKey: eatery.name) {
            gradientView.updateFrame()
            placeImage = cachedImage
        } else {
            cancellable = API.PlaceSearch.loadPlace(name: eatery.name, queryType: .placeByName)?
                ...
                .sink(receiveCompletion: { _ in }, receiveValue: { [weak self] image in
                    ...
                    ImageCache.shared.set(image, forKey: eatery.name)
                })
        }
    }
